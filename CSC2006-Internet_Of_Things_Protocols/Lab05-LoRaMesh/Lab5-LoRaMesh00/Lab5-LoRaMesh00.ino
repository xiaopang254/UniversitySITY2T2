...
void processDataMessage() 
{
  ...
  display.print(F("Recv'd data message"));
  display.display();
  ...
  // the '00000' is just for padding between the different messages
  ...
  if (destinationNode == "00"){
    ...
    display.setCursor(0, 24); //Overwrite the screen
    display.setTextColor(WHITE, BLACK); //Overwrite the screen
    display.print("Data: " + String(message));
    display.display();
  }
...
void createDataMessage() // read A0 and A1 create data string
{
  ...
  for (i = 0; i < nNodes; i++) {
    ...
    display.setCursor(0, 16); //Overwrite the screen
    display.setTextColor(WHITE, BLACK); //Overwrite the screen 
