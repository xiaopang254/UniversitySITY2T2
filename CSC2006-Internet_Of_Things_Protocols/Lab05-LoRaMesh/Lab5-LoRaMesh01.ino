...
void processDataMessage() 
{
  ...
  // check if destination node is current node id which is node 1
  // and source node is previous node id which is 0
  if (destinationNode == "01" && sourceNode == "00"){
    Serial.println("Message received is: " + stringMid(receivedMessage, 10, 4));

    display.setCursor(0, 0);  // write at the first line
    display.setTextColor(WHITE, BLACK);   // overwrite the screen
    // source node ID which is suppose to be node 0
    ...
    // store the message received from node 0
